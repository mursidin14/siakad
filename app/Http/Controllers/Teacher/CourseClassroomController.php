...
class CourseClassroomController extends Controller
{
    ...
    public function index(Course $course, ClassRoom $classroom): Response
    {
        ...
        ->with([
            ...
            'grades' => fn($query) => $query->where('course_id', $course->id)->where('class_room_id', $classroom->id),
        ])
        ->withCount([
            'attendances' => fn($query) => $query->where('course_id', $course->id)->where('class_room_id', $classroom->id),
        ])
        ...
        return inertia('Teacher/Classrooms/Index', [
            'page_settings' => [
                ...
                'method' => 'PUT',
                'action' => route('teacher.classrooms.sync', [$course, $classroom])
            ],
    ...
    public function sync(Course $course, ClassRoom $classroom, CourseClassroomRequest $request): RedirectResponse
    {
        try{
            ...
            flashMessage('Berhasil melakukan perubahan', 'success');
            return to_route('teacher.classrooms.index', [$course, $classroom]);
